...
function bindReagendaButtons() {
  document.querySelectorAll('.openReagendaBtn').forEach(btn => {
    if (btn.__reagendaBound) return;
    btn.__reagendaBound = true;
...
function getFilteredServicos() {
  ...
  if (!svcFilterText.trim()) {
    return [{ nome: 'Selecionar...', valor: null, placeholder: true }, ...base];
  }
...
function renderListaServicos() {
  ...
  servicoLista.innerHTML = items.map((s) => {
    ...
    const sub = s.placeholder
      ? ''
      : `<div class="svc-muted">${s.valor != null ? toBRL(s.valor) : ''}</div>`;
